...
getSInits <- function( AllDetections = AllDetections,
  ...
  for(i in 1:n.detected){
    ...
    for(t in whereDetections){
        ...
        if(nrow(tmp)>1){
        s[Id.vector[i],,t] <- colMeans(cbind(tmp$x, tmp$y))
        }else{
          s[Id.vector[i],,t] <- c(tmp$x, tmp$y)
        ...
        if(sxyID==0){
          ...
          s[Id.vector[i],,t] <- as.numeric(lowerCoords[which.minhab,])
    ...
    if(sum(whichYearDet%in%1)==1){
      ...
      s[Id.vector[i],,t] <- crbernppACmovement_normal(n=1
                                          ,lowerCoords = lowerCoords
                                          , upperCoords = upperCoords
                                          , s = as.numeric(s[Id.vector[i], 1:2, t - 1])
                                          , sd = 4
      ...
      s[Id.vector[i],,t-1] <- crbernppACmovement_normal(n=1
                                          ,lowerCoords = lowerCoords
                                          , upperCoords = upperCoords
...
for(i in 1:n.augmentedId){
  ...
  #t>1
    for(t in 2:n.years){
      s[aug.vect[i],,t] <-  crbernppACmovement_normal(n=1
                                          ,lowerCoords = lowerCoords
                                          , upperCoords = upperCoords
