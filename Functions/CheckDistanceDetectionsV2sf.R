...
CheckDistanceDetectionsV2sf <- function(  y = y
   ...
   if(method=="pairwise"){
      ...
      for(i in 1:dim(y)[1]){
         ...
         dist.det <- units::set_units(dist.det,NULL) 
         
         n.too.far <- apply(dist.det,2,function(x){
         sum(x>= max.distance)
      })
         # dist.det[,1]>units::set_units(max.distance,"m")
         # 
         to.remove <- NULL  
       ...
       combos <- lapply(1:(nrow(this.det)-1),function(x){
         ...
         temp3 <- lapply(1:length(combos), function(x) {
            ...
            out <- apply(combos[[x]], 2, function(xx){
               ...
               tmp <- st_distance(this.det2, this.det2, byid = T)
               tmp <- units::set_units(tmp,NULL) 
               
               if(!any(tmp>=max.distance)){
      ...
      if(plot.check){
         if(!is.null(to.remove)){
            ...
            plot(this.det[to.remove,]$geometry, add=TRUE, pch=19, col="red", cex=0.8)
            box()
         }
      }
